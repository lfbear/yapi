...
module.exports = function (options) {
  ...
  this.commons.saveLog = function () {
    ...
    try {
      yapi.commons.log('yapi-plugin-wecom: 开始运行');
      const logData = args[0];
      if (!logData || logData.type != 'project') {
        yapi.commons.log('yapi-plugin-wecom: 日志不是 project 类型，跳过通知。');
        return;
      }
  ...
  yapi.connect.then(function () {
    let db = mongoose.connection.db.collection('wecom_robots');
    db.createIndex({
      project_id: 1
    });
  });
